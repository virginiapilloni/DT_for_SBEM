        ...
        with open(csv_file, mode="a", newline="") as file:
            ...
            for topic, value in latest_data.items():
                ...
                if isinstance(value, dict):
                    ...
                    for key,val  in value.items(): #["apower", "voltage", "current", "output", "temperature", "aenergy"]:
                        #if key in value:
                            writer.writerow([current_time.strftime("%Y-%m-%d %H:%M:%S"), topic, key, val])
                else:
